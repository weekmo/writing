...
=======
In ICP (in our case) one points cloud (i.e., vertex cloud), or target, is kept fixed, while the template is transformed to best match the target. The algorithm iteratively check the transformation (combination of translation, rotation and scaling) required to minimize a distance between the template and the target points cloud \cite{Zhang1994}.
>>>>>>> 9a538e78ff32f01ee239aaf4f44c855a18a7dde9
...
PCA, a statistical technique converts high dimensional data to low dimensional data by selecting the most significant features that can capture maximum information about the dataset. With dimensionality reduction using PCA, the training time of the algorithms reduces and it makes the data analysis possible which is not always the case with data in high dimensions.
...
In this PCA is used in the code as a preliminary step, so that the template and target are aligned as much as possible before the registration can begin. We calculate the first principle component of each graph and align them accordingly.
>>>>>>> 9a538e78ff32f01ee239aaf4f44c855a18a7dde9
